...
def main(ratio_file, chroms, blacklist_chroms, pcs=50):
    ...
    for dic in stream_table(gzip.open(ratio_file, 'rt'),' '):
        ...
        if len(chrom.split(":")) == 4:
            chr_, s, e, clu = chrom.split(":")
        elif len(chrom.split(":")) == 5:
            chr_, s, e, clu, annotation = chrom.split(":")
        else:
            raise ValueError("Intron names have unexpected number of : delimited fields; intron names require either 4 or 5 fields")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Prepare phenotype table from leafcutter counts for QTL mapping with fastQTL."
    )
    parser.add_argument("counts_file", help="Intron usage counts file (input_perind.counts.gz), output from leafcutter-cluster.")
    parser.add_argument("-p", "--pcs", dest="npcs", type=int, default=50, help="Number of PCs to output [default %(default)s]")
    parser.add_argument("--chromosome_blacklist", default="", help="File of chromosomes to exclude from analysis, one per line. If not provided, defaults to blacklisting X and Y.")

    args = parser.parse_args()
    main(args.counts_file, get_chromosomes(args.counts_file), get_blacklist_chromosomes(args.chromosome_blacklist), args.npcs)
